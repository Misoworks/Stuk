...
pub(crate) struct WindowsDesktopServices;

#[cfg(not(target_os = "windows"))]
impl WindowsDesktopServices {
...
mod native {
    ...
    impl std::fmt::Debug for WindowsDesktopServices {
        fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            formatter
                .debug_struct("WindowsDesktopServices")
                .field(
                    "events",
    ...
    impl Default for WindowsDesktopServices {
        fn default() -> Self {
            Self {
    ...
    fn tray_image(icon: &TrayIcon) -> Option<tray_icon::Icon> {
        if let Some(path) = icon.icon_path.as_deref() {
            ...
            if let Ok(native) = tray_icon::Icon::from_path(path, None) {
                return Some(native);
            }
        }
    ...
    fn credential_entry(key: &CredentialKey) -> Option<keyring_core::Entry> {
        if !ensure_credential_store() {
            return None;
        }
        keyring_core::Entry::new(&key.service, &key.account).ok()
    }

    fn ensure_credential_store() -> bool {
        static STORE_READY: OnceLock<bool> = OnceLock::new();
        *STORE_READY.get_or_init(|| match windows_native_keyring_store::Store::new() {
            Ok(store) => {
...
#[cfg(target_os = "windows")]
pub(crate) use native::WindowsDesktopServices;
